 ...
 * @license     : GPL
 * @created     : 星期一 6月 22, 2020 09:21:45 CST
 */
...
// string -> f(value,) -> vector<mask>


map<unsigned long long ,vector<pair<string,ll> > > m2v; // mask 2 value

const int MI_N = 15;

ll ten[MI_N+10];
vector<ll>sq[MI_N+10];
...
int main(){
  ...
  rep(i,0,N){
    ...
    for(auto v:sq[n-1]){
      ...
      rep(ch,'A','Z'+1){
        rep(j,0,n){
          if(s[i][j] != ch)continue;
          int newv = (v/ten[n-1-j])%10;
          if(!c2i.count(ch)){
            c2i[ch] = newv;
          }else if(c2i[ch] != newv){
            ok = false;
  ...
  for(auto item:m2v){
    if(item.second.size() > 1){
      ...
      for(auto k:item.second){
        printf("{%s,%lld}",k.first.c_str(),k.second);
      }
      printf("\n");
    }
  }
